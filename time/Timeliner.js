  ...
  Timeliner = (function() {
    ...
    Timeliner.prototype.addPartyConnections = function(d, i, element) {
      ...
      return d3.selectAll("#correspondent_" + d.id).each(function(eD, eI) {
        ...
        return window.vis.append("svg:line").attr("x1", start.x).attr("y1", start.y).attr("x2", start.x).attr("y2", start.y).style("stroke-width", 0.45).attr("class", connectionClass).transition().duration(400 + (Math.max(stop.x, start.x) / 2)).attr("x2", stop.x).attr("y2", stop.y);
      });
    };
    ...
    Timeliner.prototype.addExchangeConnections = function(d, i, element) {
      ...
      return d3.select(element).selectAll("g.exchange").each(function(d, i) {
        var exchangePt;
        exchangePt = self.circleToGlobalsCoords(this.childNodes[0]);
        return d3.select("#party_" + d.sor).each(function(dp, i) {
          ...
          return window.vis.append("svg:line").attr("x1", start.x).attr("y1", start.y).attr("x2", start.x).attr("y2", start.y).style("stroke-width", 0.45).attr("class", connectionClass).transition().duration(600).attr("x2", stop.x).attr("y2", stop.y);
        });
      });
    };
    ...
    Timeliner.prototype.plotCases = function(cases) {
      ...
      cases.forEach(function(d) {
        ...
        return d.last_item = parse(d.last_item);
      });
      x.domain([
      ...
      }).attr("height", barHeight).on("click", function(d, i) {
        return window.open("http://web102881.pbe.oslo.kommune.no/saksinnsyn/casedet.asp?direct=Y&mode=all&caseno=" + d.doc);
      }).transition().attr("width", function(d, i) {
        return x(d.last_item) - x(d.initiated_at);
      }).delay(function(d, i) {
        return i * 1 / 2;
      });
    ...
    Timeliner.prototype.plotParties = function(parties) {
      ...
      return node.append("svg:text").attr("text-anchor", "middle").attr("dy", ".3em").attr("font-size", ".6em").attr("letter-spacing", "-0.07em").attr("font-weight", "500").text(function(d) {
